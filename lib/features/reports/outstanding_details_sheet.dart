...
// It only depends on the Outstanding model + PdfService, and keeps
// its own copy of the palette below. Edit freely; nothing in
// outstanding_screen.dart reaches into this file's internals beyond
// the OutstandingDetailsSheet(item, isCust) constructor.
// ------------------------------------------------------------------

// --- PALETTE (kept local on purpose — see note above) ---------------
...
const _kCustAccent = Color(0xFF0F6E56); // Customers / receivable
const _kVendAccent = Color(0xFF0C447C); // Vendors / payable
...
class OutstandingDetailsSheet extends StatelessWidget {
  ...
  Future<void> _shareOutstandingDetails(BuildContext context) async {
    try {
      ...
      await PdfService.generateAndShareReport(
        ...
        headers: const ['Invoice No', 'Date', 'Amount'],
        data: reportData,
      );
    } catch (e) {
      if (context.mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final total = item.outstandingAmount ?? 0.0;
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    // --- INVOICE BREAKDOWN (all records for this account) ---
                    Padding(
                        padding: const EdgeInsets.only(bottom: 8, left: 2),
                        child: Text(
                          'UNPAID INVOICES',
                          style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: _kMuted, letterSpacing: 0.8),
                        ),
                      ),
                      Container(
                        ...
                        child: item.invoices.isEmpty
                            ...
                            : Builder(
                                builder: (context) {
                                  // 1. SORT THE INVOICES: Newest first (Descending)
                                  final sortedInvoices = item.invoices.toList()..sort((a, b) {
                                    final dateA = DateTime.tryParse(a.date ?? '') ?? DateTime(1970);
                                    final dateB = DateTime.tryParse(b.date ?? '') ?? DateTime(1970);
                                    return dateB.compareTo(dateA); // b compared to a = newest first
                                  });

                                  return Column(
                                    children: List.generate(sortedInvoices.length, (i) {
                                      final inv = sortedInvoices[i];
                                      final isLast = i == sortedInvoices.length - 1;
                                      
                                      return Column(
                                        children: [
                                          // 2. MAKE IT TAPPABLE
                                          InkWell(
                                            // This ensures the tap ripple respects the container's rounded corners on top/bottom
                                            borderRadius: BorderRadius.vertical(
                                              top: i == 0 ? const Radius.circular(14) : Radius.zero,
                                              bottom: isLast ? const Radius.circular(14) : Radius.zero,
                                            ),
                                            onTap: () {
                                              // Navigate to the new Details Screen passing the Voucher ID
                                              Navigator.push(
                                                context,
                                                MaterialPageRoute(
                                                  builder: (context) => VoucherDetailsScreen(
                                                    voucherId: inv.invoiceNumber ?? '',
                                                  ),
                                                ),
                                              );
                                            },
                                              ...
                                              child: Row(
                                                ...
                                                children: [
                                                  Column(
                                                    crossAxisAlignment: CrossAxisAlignment.start,
                                                    children: [
                                                      Text('Inv ${inv.invoiceNumber ?? 'N/A'}', style: const TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600, color: _kText)),
                                                      const SizedBox(height: 3),
                                                      Text(inv.date?.split('T')[0] ?? '', style: const TextStyle(fontSize: 12, color: _kMuted)),
                                                    ],
                                                  ),
                                                  Text('Rs. ${inv.amount?.toStringAsFixed(2) ?? '0.00'}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _kInk)),
                                                ],
                                              ),
                                            ),
                                          ),
                ...
                child: SafeArea(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () => _shareOutstandingDetails(context),
                      icon: const Icon(Icons.picture_as_pdf_rounded, size: 19),
                      label: const Text('Download & Share Statement', style: TextStyle(fontSize: 14.5, fontWeight: FontWeight.w700)),

...
import 'package:sas_akount_login/models/receivable/receivable.dart';
import 'package:sas_akount_login/features/receivable/receivable_details_sheet.dart';

// --- PALETTE (shared with sale_screen.dart / outstanding_screen.dart) ---
...
// Receivable = money coming in → same family as Sales / Outstanding-Customers.
...
class _ReceivableScreenState extends State<ReceivableScreen> {
  ...
  void _loadData() {
    setState(() {
      _futureData = _apiService.fetchReceivables().then((response) {
        final records = response['records'];
        if (records is! List) {
          return <Receivable>[];
        }
        return records.map((item) {
          return Receivable.fromMap(Map<String, dynamic>.from(item as Map));
        }).toList();
      });
    });
  }
  ...
  void _showReceivableDetails(BuildContext context, Receivable item) {
    showModalBottomSheet(
      ...
      builder: (context) => ReceivableDetailsSheet(item: item),
    );
  }

  double _sumTotal(List<Receivable> items) {
    double sum = 0;
    for (final i in items) {
      sum += i.receivableAmount ?? 0.0;
    }
    return sum;
  }

  Widget _buildSummaryStrip(List<Receivable> items) {
  ...
  Widget _buildRow(Receivable item, bool showDivider) {
    final title = item.customerName ?? 'Unknown Customer';
    final subtitle = 'ID: ${item.id ?? 'N/A'}';
    final amount = item.receivableAmount ?? 0.0;
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Receivables', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w800, color: _kInk, letterSpacing: -0.4)),
                  InkWell(
                    onTap: _loadData,
            ...
            // --- CONTENT ---
            Expanded(
              child: FutureBuilder<List<Receivable>>(
                future: _futureData,
                    ...
                    return Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 6),
                          const Text('No outstanding receivables', style: TextStyle(color: _kMuted, fontSize: 13.5)),
                        ],
                      ),
                    );
                  }

...
class _PurchaseEntryScreenState extends State<PurchaseEntryScreen> {
  ...
  void _initializeDates() {
    ...
    _dateController.text =
        NepaliDateFormat('dd/MM/yyyy').format(_selectedNepaliDate);
  }
  Future<void> _fetchNextVoucher() async {
    try {
      final String nextVoucher =
          await _purchaseService.getNextPurchaseVoucher();
      if (!mounted) return;
      setState(() {
        _voucherNoController.text =
            nextVoucher.isNotEmpty ? nextVoucher : 'Error';
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
        _voucherNoController.text = 'Error';
      });
    }
  }
  Future<void> _fetchTermMasters() async {
    try {
      final List<dynamic> terms =
          await _purchaseService.fetchPurchaseTermMasters();
      if (terms.isEmpty || !mounted) return;
      setState(() {
        _termMasters = terms;
        _billWiseTerms = terms.where((t) {
          final itemWiseValue =
              t['ItemWise'] ?? t['itemWise'] ?? t['itemwise'];
          final String isItemWise =
              itemWiseValue?.toString().trim().toUpperCase() ?? 'N';
          return isItemWise == 'N' ||
              isItemWise == '0' ||
              isItemWise == 'FALSE' ||
              isItemWise == '';
        }).map<Map<String, dynamic>>((t) {
          final double dbRate = double.tryParse(
                (t['Rate'] ?? t['rate'] ?? 0.0).toString(),
              ) ??
              0.0;
          return {
            'termId':
        ...
        _billWiseTerms.sort((a, b) {
          final bool aIsVat =
              a['termName'].toString().toUpperCase().contains('VAT');
          final bool bIsVat =
              b['termName'].toString().toUpperCase().contains('VAT');
      ...
      debugPrint('Error fetching purchase term masters: $e');
    }
  }
  Future<void> _selectSystemDate(BuildContext context) async {
    ...
    if (picked != null && mounted) {
      setState(() {
        ...
        _dateController.text =
            NepaliDateFormat('dd/MM/yyyy').format(picked);
      });
    }
  }
  ...
  void _calculateTotals({
    ...
    for (final item in _cartItems) {
      ...
      if (item['itemTerms'] != null) {
        for (final term in item['itemTerms']) {
          if (term['termName']
              .toString()
              .toUpperCase()
              .contains('VAT')) {
      ...
      if (!hasItemVat) {
        tempTaxable += lineTotal;
      }
    }
    double runningTotal = tempSub;
    double runningTaxable = tempTaxable;
    for (int i = 0; i < _billWiseTerms.length; i++) {
      final Map<String, dynamic> bt = _billWiseTerms[i];
      if (manualIndex == i) {
        bt['isManualAmount'] = !isPercentChange;
      }
      if (bt['isActive'] == true) {
        final bool isVat =
            bt['termName'].toString().toUpperCase().contains('VAT');
        final double base = isVat ? runningTaxable : runningTotal;
        final bool isManual = bt['isManualAmount'] ?? false;
        double amount = 0.0;
        if (isManual) {
          amount = double.tryParse(bt['amountController'].text) ?? 0.0;
          if (base > 0 && !isPercentChange) {
            final double percent = (amount / base) * 100;
            bt['percentController'].text = _cleanNum(percent);
          }
        } else {
          final double percent =
              double.tryParse(bt['percentController'].text) ?? 0.0;
          amount = (base * percent) / 100;
          bt['amountController'].text =
              amount == 0 ? '0' : amount.toStringAsFixed(2);
        }
        if (bt['sign'] == '-') {
          runningTotal -= amount;
          if (!isVat) {
            runningTaxable -= amount;
          }
        } else {
          runningTotal += amount;
          if (!isVat) {
    ...
    setState(() {
      ...
      _grandTotal = runningTotal.clamp(0.0, double.infinity).toDouble();
    });
  }
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (isSelected)
              Icon(Icons.check_circle_rounded, color: _primaryColor, size: 22),
          ],
        ),
      ),
    );
  ...
  void _showVendorSelection() {
    final Future<List<Map<String, dynamic>>> vendorsFuture =
        _purchaseService.getPurchaseVendors();
              ...
              builder: (ctx, setModal) {
                return FutureBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (ctx, snapshot) {
                    ...
                    final filtered = all.where((vendor) {
                      final name = (vendor['LedgerName'] ??
                              vendor['ledgerName'] ??
                              '')
                          .toString()
                          .toLowerCase();
                      final code = (vendor['LedgerCode'] ??
                              vendor['ledgerCode'] ??
                              '')
                          .toString()
                          .toLowerCase();
                              ...
                              : ListView.builder(
                                  ...
                                  itemBuilder: (_, i) {
                                    ...
                                    final code = (vendor['LedgerCode'] ??
                                        ...
                                        .toString();
                                    final selectedId =
                                        _selectedVendor?['LedgerID'] ??
                                            _selectedVendor?['ledgerID'];
                                    final vendorId =
                                        vendor['LedgerID'] ?? vendor['ledgerID'];
                                    ...
                                    return ListTile(
                                      ...
                                      title: Text(name),
                                      subtitle:
                                          code.isNotEmpty ? Text(code) : null,
                                      trailing: isSelected
                                          ? Icon(
                                              Icons.check_circle,
                                              color: _primaryColor,
  ...
  void _showAddBillingItemDialog(Map<String, dynamic> item) {
    ...
    final rateCtrl = TextEditingController(
      text: initRate.toStringAsFixed(2),
    );
    List<Map<String, dynamic>> dialogTerms = _termMasters.where((t) {
      ...
      return {
        'termId':
            t['TermID'] ?? t['termID'] ?? t['termid'] ?? t['TermId'] ?? 0,
        'termName':
            (t['TermName'] ?? t['termName'] ?? t['termname'] ?? 'Term')
                .toString(),
        'sign': (t['Sign'] ?? t['sign'] ?? '+').toString().trim(),
              ...
              for (int i = 0; i < dialogTerms.length; i++) {
                ...
                if (term['isActive'] == true) {
                  if (fromAmount && index == i) {
                    final double amount =
                        double.tryParse(term['amountController'].text) ?? 0.0;
                    if (currentTotal > 0) {
                      term['percentController'].text =
                          _cleanNum((amount / currentTotal) * 100);
                    }
                    if (term['sign'] == '-') {
                      currentTotal -= amount;
                    } else {
                    ...
                    final double percent =
                        double.tryParse(term['percentController'].text) ?? 0.0;
                    final double amount = (currentTotal * percent) / 100;
                    term['amountController'].text =
                        amount == 0 ? '0' : amount.toStringAsFixed(2);
                    if (term['sign'] == '-') {
                      currentTotal -= amount;
                    } else {
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: TextField(
                                  ...
                                  decoration:
                                      _modernInputDecoration(prefix: 'Rs '),
                                  keyboardType: TextInputType.number,
                                  onChanged: (_) => recalculateDialog(
                                    fromAmount: true,
                                    index: idx,
                        ...
                        'itemTerms': dialogTerms
                          ...
                          return {
                            ...
                            'percent': double.tryParse(
                                  dt['percentController'].text,
                                ) ??
                                0.0,
                            'amount': double.tryParse(
                                  dt['amountController'].text,
                                ) ??
                                0.0,
                          };
                        }).toList(),
  ...
  void _showEditItemDialog(int index) {
    ...
    if (ci['itemTerms'] != null) {
      for (final existingTerm in ci['itemTerms']) {
        for (final dt in dialogTerms) {
          if (dt['termId'] == existingTerm['termId']) {
            dt['isActive'] = true;
            dt['percentController'].text = _cleanNum(
              double.tryParse(existingTerm['percent'].toString()) ?? 0.0,
            );
    ...
    double dialogNetTotal =
        double.tryParse(ci['totalAmount'].toString()) ?? 0.0;
    showGeneralDialog(
      context: context,
      barrierDismissible: true,
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: () {
                    setState(() {
                      ...
                      _cartItems[index]['itemTerms'] = dialogTerms
                          .where((dt) => dt['isActive'] == true)
                          .map((dt) {
                        return {
                          'termId': dt['termId'],
  ...
  Future<void> _submitPurchase() async {
    ...
    if (_paymentModeLabel == 'Credit' && _selectedVendor == null) {
      _showSnack(
        'Please select a Vendor for a Credit purchase.',
        isError: true,
      );
      return;
    }
    ...
    final ledgerId = _selectedVendor != null
        ? (_selectedVendor!['LedgerID'] ?? _selectedVendor!['ledgerID'])
        : null;
    final vendorName = _selectedVendor != null
        ? (_selectedVendor!['LedgerName'] ??
                _selectedVendor!['ledgerName'] ??
                '')
            .toString()
        : _cashPartyNameController.text.trim();
    final payload = {
      'vendorLedgerId': ledgerId,
      'vendorName': vendorName.isNotEmpty ? vendorName : null,
      'partyBillNo': _supplierBillNoController.text.trim(),
      'partyBillDate': _selectedSupplierAdDate != null
          ? DateFormat('yyyy-MM-dd').format(_selectedSupplierAdDate!)
          : null,
      'branchId': null,
      ...
      'billTerms': _billWiseTerms
        ...
        return {
          ...
          'sign': t['sign'],
          'percent':
              double.tryParse(t['percentController'].text) ?? 0.0,
          'amount':
              double.tryParse(t['amountController'].text) ?? 0.0,
        };
      }).toList(),
    };
    try {
      final result = await _purchaseService.submitPurchase(payload);
      if (!mounted) return;
      setState(() {
        _isSubmitting = false;
        _isSaving = false;
  ...
  void _showSuccessSheet(Map<String, dynamic> result) {
    ...
    final dynamic rawNetAmount = result['netAmount'] ?? _grandTotal;
    final double netAmount = rawNetAmount is num
        ? rawNetAmount.toDouble()
        : double.tryParse(rawNetAmount.toString()) ?? _grandTotal;
    showModalBottomSheet(
      context: context,
      isDismissible: false,
      enableDrag: false,
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    _summaryRow(
                      'Supplier Bill No',
                      _supplierBillNoController.text.trim().isEmpty
                          ? 'N/A'
                          : _supplierBillNoController.text.trim(),
                    ),
                  ],
                ),
              ),
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
              ...
              borderSide: BorderSide(
                color: onTap != null ? _primaryColor : Colors.grey[300]!,
              ),
            ),
          ),
        ),
  ...
  Widget build(BuildContext context) {
    final String vendorName = _selectedVendor == null
        ? 'Select Vendor / Supplier *'
        : (_selectedVendor!['LedgerName'] ??
                _selectedVendor!['ledgerName'] ??
                'Unknown Vendor')
        ...
        child: Column(
          ...
          children: [
            ...
            Align(
              ...
              child: Text(
                'AD: ${DateFormat('dd MMM yyyy').format(_selectedAdDate)}',
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField(
                          label: 'Supplier Bill No (Optional)',
          ...
          child: SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: _isSaving
                  ...
                  : Text(
                      'Save Purchase - Rs. ${_grandTotal.toStringAsFixed(2)}',
                      style: const TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
                        color: Colors.white,
  ...
  Widget _buildVendorCard(String vendorName) {
    final String ledgerCode = (_selectedVendor?['LedgerCode'] ??
            _selectedVendor?['ledgerCode'] ??
            '')
        .toString();
    return Container(
      ...
      child: Row(
        children: [
          CircleAvatar(
            radius: 20,
            backgroundColor: _selectedVendor == null
                ? Colors.blue.shade50
                : _primaryColor.withValues(alpha: 0.1),
            child: Icon(
              _selectedVendor == null
        ...
        children: [
          Container(
            ...
            child: Row(
              ...
              children: [
                Text(
                  'Purchase Items (${_cartItems.length})',
                  style: const TextStyle(
                    fontSize: 15,
                    fontWeight: FontWeight.bold,
                    color: Colors.black87,
  ...
  Widget _buildCartItemTile(int i) {
    ...
    final double discount =
        (gross - total).clamp(0.0, double.infinity).toDouble();
    final double discountPercent = gross > 0 ? (discount / gross) * 100 : 0;
    return Padding(
      padding: const EdgeInsets.all(14),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  '${qty % 1 == 0 ? qty.toInt() : qty} x Rs. ${price.toStringAsFixed(2)}',
                  style: TextStyle(color: Colors.grey[600], fontSize: 12),
                ),
                if (discount > 0) ...[
                  const SizedBox(height: 4),
                  Row(
                    children: [
                      ...
                      Text(
                        'Terms Applied: Rs. ${discount.toStringAsFixed(2)} (${discountPercent.toStringAsFixed(1)}%)',
                        style: TextStyle(
                          fontSize: 11,
                          color: Colors.orange.shade700,
                        ),
  ...
  Widget _buildBillTermRow(int index) {
    final Map<String, dynamic> bt = _billWiseTerms[index];
    final bool isActive = bt['isActive'] == true;
  ...
  Widget _buildTotalCard() {
    final activeNonVatTerms = _billWiseTerms
        .where(
          (t) =>
              t['isActive'] == true &&
              !t['termName'].toString().toUpperCase().contains('VAT'),
        )
        .toList();
    final activeVatTerms = _billWiseTerms
        .where(
          (t) =>
              t['isActive'] == true &&
              t['termName'].toString().toUpperCase().contains('VAT'),
    ...
    return Column(
      ...
      children: [
        ...
        ...List.generate(_billWiseTerms.length, (index) {
          if (_billWiseTerms[index]['termName']
              .toString()
              .toUpperCase()
              .contains('VAT')) {
            ...
            children: [
              ...
              if (activeNonVatTerms.isNotEmpty) ...[
                ...
                ...activeNonVatTerms.map((t) {
                  ...
                  final bool isDeduction = t['sign'] == '-';
                  return _summaryRow(
                    t['termName'].toString(),
                    '${isDeduction ? '- ' : '+ '}Rs. ${amount.toStringAsFixed(2)}',
                    valueColor: isDeduction
              ...
              _summaryRow(
                'Taxable Amount',
                'Rs. ${_taxableAmount.toStringAsFixed(2)}',

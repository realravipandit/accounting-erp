...
class PurchaseOrderEntryScreen extends StatefulWidget {
  const PurchaseOrderEntryScreen({super.key});

  @override
  State<PurchaseOrderEntryScreen> createState() => _PurchaseOrderEntryScreenState();
}

class _PurchaseOrderEntryScreenState extends State<PurchaseOrderEntryScreen> {
  final SalesService _salesService = SalesService(); // only for shared item picker
  final PurchaseOrderService _purchaseOrderService = PurchaseOrderService();

  // TODO: placeholder color, polish styling later
  final Color _primaryColor = const Color(0xFF3B82F6); // Purchase Blue
  ...
  Future<void> _fetchNextOrderNumber() async {
    try {
      final nextOrd = await _purchaseOrderService.getNextPurchaseOrderNumber();
  ...
  bool _isItemWise(dynamic t, String flag) {
    final v = (t['ItemWise'] ?? t['itemWise'] ?? '').toString().trim().toUpperCase();
    return v == flag;
  }

  Map<String, dynamic> _termToMap(dynamic t) {
    return {
      'termId': t['TermID'] ?? t['termID'] ?? t['termId'],
  ...
  Future<void> _fetchTermMasters() async {
    try {
      final terms = await _purchaseOrderService.fetchTermMasters();
      if (!mounted) return;
      setState(() {
        _termMasters = terms;
        _billWiseTerms = terms.where((t) => _isItemWise(t, 'N')).map<Map<String, dynamic>>(_termToMap).toList();
      ...
      _showSnack('Could not load terms: $e', isError: true);
    }
  }
  ...
  void _showVendorSelection() {
    final Future<List<dynamic>> vendorsFuture = _purchaseOrderService.fetchVendors();
              ...
              builder: (ctx, snapshot) {
                ...
                return Column(
                  children: [
                    ...
                    const Text('Select Vendor', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      ...
                      child: filtered.isEmpty
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (_, i) {
                                ...
                                final isSelected = _selectedVendor != null &&
                                    (_selectedVendor!['LedgerID'] ?? _selectedVendor!['ledgerID']) == (c['LedgerID'] ?? c['ledgerID']);
                                return ListTile(
  ...
  void _showAddBillingItemDialog(Map<String, dynamic> item) {
    ...
    final rawRate = item['PurchaseRate'] ?? item['purchaseRate'] ?? item['SalesRate'];
    final double initRate = rawRate != null ? double.tryParse(rawRate.toString()) ?? 0.0 : 0.0;
    final rateCtrl = TextEditingController(text: initRate.toStringAsFixed(2));

    List<Map<String, dynamic>> dialogTerms = _termMasters.where((t) => _isItemWise(t, 'Y')).map<Map<String, dynamic>>(_termToMap).toList();
                ...
                onPressed: () {
                  setState(() {
                    _cartItems.add({
                      ...
                      // Item's default unit, pulled silently from item master (no picker in UI)
  ...
  Future<void> _submitOrder() async {
    ...
    final ledgerId = _selectedVendor!['LedgerID'] ?? _selectedVendor!['ledgerID'];
    final vendorNameStr = (_selectedVendor!['LedgerName'] ?? _selectedVendor!['ledgerName'] ?? '').toString();
    ...
    try {
      final result = await _purchaseOrderService.submitPurchaseOrder(payload);
      if (!mounted) return;
  ...
  void _showSuccessSheet(Map<String, dynamic> result) {
    ...
    final netAmountRaw = result['netAmount'] ?? _grandTotal;
    final double netAmount = double.tryParse(netAmountRaw.toString()) ?? _grandTotal;
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('Purchase Order Saved!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('New Purchase Order', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.w600)),
        actions: [
          Padding(
  ...
  Widget _buildVendorCard(String vendorName) {
    ...
    final code = hasVendor ? (_selectedVendor!['LedgerCode'] ?? _selectedVendor!['ledgerCode'] ?? '').toString() : '';
    ...
    return Container(
      ...
      child: Row(children: [
        CircleAvatar(
          radius: 20,
          backgroundColor: hasVendor ? _primaryColor.withValues(alpha: 0.1) : Colors.orange.shade50,
          child: Icon(
            hasVendor ? Icons.store_rounded : Icons.store_outlined,
            color: hasVendor ? _primaryColor : Colors.orange,
            size: 20,
          ),
        ),
        const SizedBox(width: 12),
              ...
              Text(
                !hasVendor
                    ...
                    : code.isNotEmpty
                        ? 'Code: $code'
                        : 'Vendor',
                style: TextStyle(color: hasVendor ? Colors.grey[500] : Colors.orange.shade700, fontSize: 12),
              ),
            ],
          ),
        ),
        OutlinedButton(
          ...
          child: Text(hasVendor ? 'Change' : 'Select', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
        ),
      ]),
    );
  }
                    ...
                    onPressed: () {
                      showDialog(
                        ...
                        builder: (_) => AlertDialog(
                          ...
                          content: const Text('This will remove all items from the order.'),
                          actions: [

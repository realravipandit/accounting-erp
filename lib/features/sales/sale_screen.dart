            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (picked != null) {
                    // 1. Format the start and end dates nicely (e.g. "12 Oct")
                    ...
                    // 2. If it's the same day, just show one date. Otherwise, show the range.
                    final isSameDay = start.year == end.year && start.month == end.month && start.day == end.day;
                    final customLabel = isSameDay ? startStr : '$startStr - $endStr';

                    // 3. Pass our new dynamic label instead of a static string!
                    _applyQuickFilter(customLabel, picked.start, endOfDay(picked.end));
                  }
                },
              ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Rs. ${FormatUtil.formatCurrency(total.toString())}',
            ...
            if (_sales.isNotEmpty)
              Container(
                ...
                padding: EdgeInsets.fromLTRB(
                  ...
                  2,
                  16,
                  8 + (MediaQuery.of(context).padding.bottom * 0.7),
                ),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: 10),
                Text(
                  'Rs. ${FormatUtil.formatCurrency(totalStr)}', 
                  style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: _kSalesAccent)
                ),
              ],
            ),
          ),
...
class FormatUtil {
  // Pre-compiled RegExp for performance. Compiles once, used everywhere.
  static final RegExp _currencyRegExp = RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))');

  static String formatCurrency(String amount) {
    double parsed = double.tryParse(amount) ?? 0.0;
    return parsed.toStringAsFixed(2).replaceAllMapped(_currencyRegExp, (Match m) => '${m[1]},');
  }
}

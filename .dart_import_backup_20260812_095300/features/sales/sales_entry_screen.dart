...
import 'package:sas_akount_login/api_service.dart'; // Adjust this import if needed
...
class _SalesEntryScreenState extends State<SalesEntryScreen> {
  ...
  Future<void> _fetchNextInvoice() async {
    try {
      final nextInv = await _apiService.getNextInvoiceNumber();
  ...
  Future<void> _fetchTermMasters() async {
    try {
      final terms = await _apiService.fetchTermMasters();
      if (mounted) {
        setState(() {
          _termMasters = terms;
          
          // Map Bill-Wise terms (ItemWise == 'N')
          ...
          // Sort so VAT strictly appears at the bottom of the list
  ...
  void _calculateTotals({int? manualIndex, bool isPercentChange = false}) {
    ...
    for (var item in _cartItems) {
      ...
      // If the item did NOT have its own VAT applied in the popup, it goes to Overall Taxable
    ...
    for (int i = 0; i < _billWiseTerms.length; i++) {
      ...
      // Lock in whether the user prefers the manual Amount or Percentage
      if (manualIndex == i) {
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _paymentTile(
              ctx: ctx, icon: Icons.money_rounded, iconColor: Colors.green, bgColor: Colors.green.shade50,
              title: 'Cash', subtitle: 'Immediate cash payment',
              onTap: () {
                setState(() { _paymentModeLabel = 'Cash'; _paymentLedgerId = null; });
                Navigator.pop(ctx);
              },
            ),
            const SizedBox(height: 8),
            _paymentTile(
              ctx: ctx, icon: Icons.account_balance_wallet_rounded, iconColor: Colors.orange, bgColor: Colors.orange.shade50,
              title: 'Credit', subtitle: 'Payment on credit / due',
              onTap: () {
                setState(() { _paymentModeLabel = 'Credit'; _paymentLedgerId = null; });
                Navigator.pop(ctx);
              },
            ),
            const SizedBox(height: 8),
            _paymentTile(
              ctx: ctx, icon: Icons.account_balance_rounded, iconColor: Colors.blue, bgColor: Colors.blue.shade50,
              title: 'Bank / Online', subtitle: 'Bank transfer, card, QR, etc.',
              onTap: () async {
                Navigator.pop(ctx);
  ...
  Widget _paymentTile({
    required BuildContext ctx, required IconData icon, required Color iconColor,
    required Color bgColor, required String title, required String subtitle, required VoidCallback onTap,
  }) {
  ...
  Future<void> _pickBankLedger() async {
    ...
    try { ledgers = await _apiService.fetchLedgers(); } catch (_) {}
    if (!mounted) return;
  ...
  void _showCustomerSelection() {
    final Future<List<Map<String, dynamic>>> ledgersFuture = _apiService.fetchLedgers();
                ...
                return Column(
                  children: [
                    ...
                    ListTile(
                      ...
                      onTap: () {
                        setState(() { _selectedCustomer = null; _paymentModeLabel = 'Cash'; _paymentLedgerId = null; });
                        Navigator.pop(ctx);
                      },
                    ),
                      ...
                      child: filtered.isEmpty
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (_, i) {
                                ...
                                final isSelected = _selectedCustomer != null && (_selectedCustomer!['LedgerID'] ?? _selectedCustomer!['ledgerID']) == (c['LedgerID'] ?? c['ledgerID']);
                                return ListTile(
                                  ...
                                  onTap: () {
                                    setState(() { _selectedCustomer = c; _paymentModeLabel = 'Credit'; _paymentLedgerId = null; });
                                    Navigator.pop(ctx);
                                  },
                                );
                              },
                    ...
                    ...dialogTerms.asMap().entries.map((entry) {
                       ...
                       return Padding(
                         ...
                         child: Row(children: [
                           ...
                           SizedBox(width: 60, child: TextField(controller: t['percentController'], decoration: _modernInputDecoration(suffix: '%'), keyboardType: TextInputType.number, onChanged: (_) => recalculateDialog())),
                           const SizedBox(width: 8),
                           SizedBox(width: 80, child: TextField(controller: t['amountController'], decoration: _modernInputDecoration(prefix: 'Rs '), keyboardType: TextInputType.number, onChanged: (_) => recalculateDialog(fromAmount: true, index: idx))),
                         ]),
                       );
                    }),
  ...
  Future<void> _submitSale() async {
    ...
    try {
      final result = await _apiService.submitSale(payload);
      if (!mounted) return;
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  if (tenderAmount > 0) ...[const SizedBox(height: 8), _summaryRow('Tendered', 'Rs. ${tenderAmount.toStringAsFixed(2)}')],
                  if (returnAmount > 0) ...[const SizedBox(height: 8), _summaryRow('Return', 'Rs. ${returnAmount.toStringAsFixed(2)}', valueColor: Colors.green)],
                  if (balanceDue > 0) ...[const SizedBox(height: 8), _summaryRow('Balance Due', 'Rs. ${balanceDue.toStringAsFixed(2)}', valueColor: Colors.red)],
                ],
              ),
            ),
  ...
  void dispose() {
    ...
    // Dispose the new term controllers properly
    for (var t in _billWiseTerms) {
  ...
  Widget _buildCartItemTile(int i) {
    ...
    // Quick approximation for display
  ...
  Widget _buildTotalCard() {
    // Filter out only the terms that the user has actively checked
    ...
    return Column(
      ...
      children: [
        ...
        // 1. Input Rows (So user can still edit percentages/amounts)
        ...
        // 2. The Detailed Receipt / Summary Card
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Subtotal (Always at the top)
              _summaryRow('Subtotal', 'Rs. ${_subTotal.toStringAsFixed(2)}'),

              // Dynamically render active Additions/Deductions (Non-VAT)
              ...
              // Dynamically render active VAT / Taxes
              ...
              // Dark Premium Grand Total Block
              Container(
...
class ItemSearchScreen extends StatefulWidget {
  ...
  const ItemSearchScreen({super.key, required this.apiService, required this.primaryColor});

...
class _ItemMasterScreenState extends State<ItemMasterScreen> {
  ...
  final List<String> _itemTypes = ['Inventory Item', 'Service Item', 'Fixed Asset'];
  final List<String> _vatOptions = ['Yes', 'No'];
  final List<String> _existingItemNames = ['Dl40 Chinese', 'J.S. Back Stool', 'Hydrolic', 'Wheel']; 
  ...
  void _onGroupSelected(Map<String, dynamic> group) async {
    setState(() {
      ...
      _groupController.text = (group['GrpName'] ?? group['grpName'] ?? group['ItemGrpName'] ?? '').toString();
    ...
    try {
      final groupId = group['ItemGrpID'] ?? group['itemGrpID'];
      if (groupId != null) {
        final subGroups = await _apiService.getItemSubGroups(groupId);
        if (mounted) {
          setState(() {
            _itemSubGroups = subGroups;
          });
        }
      }
    } catch (e) {
  ...
  Future<void> _submitItem() async {
    ...
    try {
      if (_mfgDateController.text.isNotEmpty) {
        formattedMfgDate = DateFormat('yyyy-MM-dd').format(DateFormat('dd/MM/yyyy').parse(_mfgDateController.text));
      }
      if (_expDateController.text.isNotEmpty) {
        formattedExpDate = DateFormat('yyyy-MM-dd').format(DateFormat('dd/MM/yyyy').parse(_expDateController.text));
      }
    } catch (e) {
    ...
    if (_selectedItemType == 'Service Item') dbItemType = 'SV';
    if (_selectedItemType == 'Fixed Asset') dbItemType = 'FX';
    ...
    final itemData = {
      ...
      'ItemSubGrpID': _selectedSubGroup?['ItemSubGrpID'] ?? _selectedSubGroup?['itemSubGrpID'],
      ...
      'vatRate': _vatStatus == 'Yes' ? (double.tryParse(_rateController.text) ?? 13.0) : 0.0,
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedUnitId != null && _availableUnits.isNotEmpty) {
      try {
        primaryUnitCode = _availableUnits.firstWhere((u) => (u['unitId'] ?? u['UnitID']) == _selectedUnitId)['unitCode'] ?? '';
      } catch (_) {}
    }
    ...
    if (_selectedAltUnitId != null && _availableUnits.isNotEmpty) {
      try {
        altUnitCode = _availableUnits.firstWhere((u) => (u['unitId'] ?? u['UnitID']) == _selectedAltUnitId)['unitCode'] ?? '';
      } catch (_) {}
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Item Master', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            children: [
              ...
              Autocomplete<String>(
                optionsBuilder: (TextEditingValue textEditingValue) {
                  ...
                  return _existingItemNames.where((option) =>
                      option.toLowerCase().contains(textEditingValue.text.toLowerCase()));
                },
                onSelected: (String selection) {
                  _itemNameController.text = selection;
                  if (selection.length >= 2) {
                    _generateItemCode(selection.substring(0, 2));
                  }
                },
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      decoration: _inputDecoration('Item Code').copyWith(fillColor: Colors.grey[200]),
                      readOnly: true, 
                    ),
                  ),
              ...
              Row(
                ...
                children: [
                  Expanded(
                    child: Autocomplete<Map<String, dynamic>>(
                      displayStringForOption: (option) {
                        return (option['GrpName'] ?? option['grpName'] ?? option['ItemGrpName'] ?? '').toString();
                      },
                      optionsBuilder: (TextEditingValue textEditingValue) {
                        if (textEditingValue.text.isEmpty) return _itemGroups;
                        return _itemGroups.where((group) {
                          final name = (group['GrpName'] ?? group['grpName'] ?? group['ItemGrpName'] ?? '').toString().toLowerCase();
                  ...
                  Expanded(
                    child: Autocomplete<Map<String, dynamic>>(
                      displayStringForOption: (option) {
                         return (option['ItemSubGrpName'] ?? option['itemSubGrpName'] ?? option['SubGrpName'] ?? option.values.last ?? '').toString();
                      },
                      optionsBuilder: (TextEditingValue textEditingValue) {
                        if (_selectedGroup == null) return const Iterable<Map<String, dynamic>>.empty();
                        if (textEditingValue.text.isEmpty) return _itemSubGroups;
                        return _itemSubGroups.where((subGroup) {
                          final name = (subGroup['ItemSubGrpName'] ?? subGroup['itemSubGrpName'] ?? subGroup['SubGrpName'] ?? '').toString().toLowerCase();
                          return name.contains(textEditingValue.text.toLowerCase());
                        });
                      },
                      onSelected: (Map<String, dynamic> selection) {
                        setState(() {
                          ...
                          _subGroupController.text = (selection['ItemSubGrpName'] ?? selection['itemSubGrpName'] ?? selection['SubGrpName'] ?? '').toString();
                        });
                      },
                      fieldViewBuilder: (context, controller, focusNode, onEditingComplete) {
                        ...
                        return TextFormField(
                          ...
                          decoration: _inputDecoration(_selectedGroup == null ? 'Select Group First' : 'Item Sub Group').copyWith(
                            fillColor: _selectedGroup == null ? Colors.grey[200] : Colors.white,
                            suffixIcon: IconButton(
                              icon: const Icon(Icons.arrow_drop_down),
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<int>(
                      ...
                      items: _availableUnits.map((u) => DropdownMenuItem<int>(
                        ...
                        Expanded(
                          child: DropdownButtonFormField<int>(
                            ...
                            decoration: _inputDecoration('Alt Unit').copyWith(
                              fillColor: _enableAltUnit ? Colors.white : Colors.grey[200],
                            ),
                            items: _availableUnits.map((u) => DropdownMenuItem<int>(
                              value: (u['unitId'] ?? u['UnitID']) as int,
                              child: Text((u['unitCode'] ?? u['UnitCode'] ?? '').toString()),
              ...
              // Row 2: The Text Fields (Alt Qty swaped to LEFT, Qty swapped to RIGHT)
              Row(
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      textInputAction: TextInputAction.next,
                      enabled: _enableAltUnit, // Only enabled if checkbox clicked
              ...
              Row(
                children: [
                  Expanded(child: TextFormField(controller: _buyPriceController, keyboardType: TextInputType.number, textInputAction: TextInputAction.next, decoration: _inputDecoration('Buy Price'))),
                  const SizedBox(width: 12),
                  Expanded(child: TextFormField(controller: _salesPriceController, keyboardType: TextInputType.number, textInputAction: TextInputAction.next, decoration: _inputDecoration('Sales Price'))),
                ],
              ),
              const SizedBox(height: 12),
              Row(
                children: [
                  Expanded(child: TextFormField(controller: _mrpController, keyboardType: TextInputType.number, textInputAction: TextInputAction.next, decoration: _inputDecoration('M.R.P.'))),
                  const SizedBox(width: 12),
                  Expanded(child: TextFormField(controller: _tradePriceController, keyboardType: TextInputType.number, textInputAction: TextInputAction.next, decoration: _inputDecoration('Trade Price'))),
                  const SizedBox(width: 12),
                  Expanded(child: TextFormField(controller: _mrRateController, keyboardType: TextInputType.number, textInputAction: TextInputAction.next, decoration: _inputDecoration('M.R. Rate'))),
                ],
              ),
              const SizedBox(height: 12),
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: _inputDecoration('Rate').copyWith(
                              fillColor: _vatStatus == 'Yes' ? Colors.white : Colors.grey[200],
                            ),
                          ),
                        ),
                        const Padding(
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      readOnly: true, 
                      decoration: _inputDecoration('Mfg Date (dd/mm/yyyy)').copyWith(suffixIcon: const Icon(Icons.calendar_month)),
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      readOnly: true, 
                      decoration: _inputDecoration('Exp. Date (dd/mm/yyyy)').copyWith(suffixIcon: const Icon(Icons.calendar_month)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isLoading 
                    ...
                    : const Text('Save Item', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                ),
              ),
              const SizedBox(height: 40),
            ],
  ...
  InputDecoration _inputDecoration(String label) {
    return InputDecoration(
      ...
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Colors.blueAccent, width: 2)),

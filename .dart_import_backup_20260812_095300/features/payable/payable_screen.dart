...
import 'package:sas_akount_login/models/payable.dart';
...
class _PayableScreenState extends State<PayableScreen> {
  ...
  final Color _themeColor = const Color(0xFFF43F5E); // Rose Red
  ...
  void _loadData() => setState(() {
    _futureData = _apiService.fetchPayables().then((data) => data.map((j) => Payable.fromMap(j)).toList());
  });
  ...
  void _showPayableDetails(BuildContext context, Payable item) {
    showModalBottomSheet(
      ...
      builder: (context) {
        return PayableDetailsSheet(item: item, themeColor: _themeColor);
      },
    );
  }

  Widget _buildCard(Payable item) {
          ...
          child: Padding(
            ...
            child: Row(children: [
              Container(
                ...
                child: Icon(Icons.arrow_upward_rounded, color: _themeColor, size: 24),
              ),
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text('TO PAY', style: TextStyle(color: _themeColor, fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 0.5)),
                const SizedBox(height: 4),
                TweenAnimationBuilder<double>(
                  tween: Tween(begin: 0, end: item.payableAmount ?? 0.0),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Account Payables', style: TextStyle(color: Color(0xFF0F172A), fontSize: 18, fontWeight: FontWeight.w700)),
        actions: [IconButton(icon: Icon(Icons.refresh_rounded, color: _themeColor), onPressed: _loadData)],
      ),
      body: FutureBuilder<List<Payable>>(
        future: _futureData,
        builder: (context, snap) {
          ...
          if (!snap.hasData || snap.data!.isEmpty) return _buildStateInfo(Icons.check_circle_outline_rounded, const Color(0xFF9CA3AF), 'All Clear!', 'No outstanding payables.');
...
class PayableDetailsSheet extends StatelessWidget {
  final Payable item;
  final Color themeColor;

  const PayableDetailsSheet({super.key, required this.item, required this.themeColor});
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow('Vendor Name', item.vendorName),
            _buildDetailRow('Ledger ID', item.id),
            _buildDetailRow('Total Payable', 'Rs. ${item.payableAmount?.toStringAsFixed(2) ?? '0.00'}', isHighlight: true, color: themeColor),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () => _sharePayableDetails(context, item),
              ),
            ),
          ],
        ),
  ...
  Future<void> _sharePayableDetails(BuildContext context, Payable item) async {
    try {
      ...
      await PdfService.generateAndShareReport(
        title: 'Payables Statement: ${item.vendorName}',
